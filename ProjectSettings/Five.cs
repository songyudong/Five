...
{
    static int[] quickPow = { 1, 10, 100, 1000, 10000, 100000 };
    public enum Piece
    {
        EMPTY = 0,
        BLACK = 1,
    ...
    public float CalBlankScore(int m, int n, Piece faction, int x_direct, int y_direct)
    {
        ...
        for (int offset = -5; offset <= 0; offset++)
        {
            ...
            for (int index = shape_score.Count - 1; index >= 0; index--)
            {
                ...
                if (Equal5(value5, item) || Equal6(value6, item))
                {
    ...
    public bool Equal5(int value, Shape s)
    {
        return s.pieces.Count == 5 && value == s.piecesValue;
    }

    public bool Equal6(int value, Shape s)
    {
        return s.pieces.Count == 6 && value == s.piecesValue;
    }
